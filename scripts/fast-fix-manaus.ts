...
async function fastFixManaus() {
  ...
  console.log(`Verificando ${rows.length} relatórios...`);
  ...
  for (const r of rows) {
    ...
    const isChanged =
      JSON.stringify(cleanedEf) !== JSON.stringify(r.efetivo) ||
      JSON.stringify(cleanedRec) !== JSON.stringify(r.recursos) ||
      JSON.stringify(cleanedInc) !== JSON.stringify(r.incendios) ||
      JSON.stringify(cleanedOut) !== JSON.stringify(r.outras);
  ...
  console.log(`\n✅ Higienização concluída! ${updatedCount} relatórios alterados no Supabase.`);
}

fastFixManaus().catch(console.error);
