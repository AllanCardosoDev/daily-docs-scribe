...
export type ExportCategory = "todos" | "incendios" | "outras" | "efetivo" | "recursos";
...
const CATEGORY_NAMES: Record<ExportCategory, string> = {
  todos: "GERAL COMPLETO (TODAS AS SEÇÕES)",
  incendios: "INCÊNDIOS (URBANO, FLORESTAL E FOCOS)",
  outras: "ATENDIMENTOS E OCORRÊNCIAS DIVERSAS",
  efetivo: "EFETIVO OPERACIONAL EMPREGADO",
  recursos: "RECURSOS OPERACIONAIS EMPREGADOS",
};

/** Relatório detalhado — tabelas por município filtradas pela categoria selecionada. */
export function buildAnnualIncendiosDoc(
  years: AnnualYearSummary[],
  category: ExportCategory = "todos",
): jsPDF {
  ...
  years.forEach((y, idx) => {
    ...
    const titleCat = CATEGORY_NAMES[category] || CATEGORY_NAMES.todos;
    drawHeader(
      doc,
      pageW,
      `RELATÓRIO OPERACIONAL POR MUNICÍPIO — ANO ${y.year}`,
      `Corpo de Bombeiros Militar do Amazonas · ${titleCat}`,
    );

    let startY = 88;

    // --- SEÇÃO 1: INCÊNDIOS ---
    if (category === "todos" || category === "incendios") {
      doc.setFont("helvetica", "bold");
      doc.setFontSize(10);
      doc.setTextColor(BRAND.r, BRAND.g, BRAND.b);
      doc.text(`1. OCORRÊNCIAS DE INCÊNDIOS (${periodLabel(y)})`, 40, startY);
    ...
    if (category === "todos" || category === "outras") {
      if (category === "todos" && startY > 700) {
        doc.addPage();
        startY = 80;
      }
      ...
      doc.text(
        category === "todos"
          ? `2. ATENDIMENTOS E OCORRÊNCIAS DIVERSAS`
          : `ATENDIMENTOS E OCORRÊNCIAS DIVERSAS (${periodLabel(y)})`,
        40,
        startY,
      );
    ...
    if (category === "todos" || category === "efetivo") {
      if (category === "todos" && startY > 700) {
        doc.addPage();
        startY = 80;
      }
      ...
      doc.text(
        category === "todos"
          ? `3. EFETIVO OPERACIONAL EMPREGADO POR MUNICÍPIO`
          : `EFETIVO OPERACIONAL EMPREGADO (${periodLabel(y)})`,
        40,
        startY,
      );
    ...
    if (category === "todos" || category === "recursos") {
      if (category === "todos" && startY > 700) {
        doc.addPage();
        startY = 80;
      }
      ...
      doc.text(
        category === "todos"
          ? `4. RECURSOS OPERACIONAIS EMPREGADOS POR MUNICÍPIO`
          : `RECURSOS OPERACIONAIS EMPREGADOS (${periodLabel(y)})`,
        40,
        startY,
      );
...
/** Resumo executivo consolidado comparativo filtrado pela categoria selecionada. */
export function buildConsolidatedIncendiosDoc(
  years: AnnualYearSummary[],
  category: ExportCategory = "todos",
): jsPDF {
  const doc = new jsPDF({ orientation: "portrait", unit: "pt", format: "a4" });
  ...
  drawHeader(
    ...
    `Síntese Acumulada (${range}) · ${titleCat}`,
  );
  ...
  if (category === "todos" || category === "incendios") {
    ...
    doc.setFontSize(10);
    doc.setTextColor(BRAND.r, BRAND.g, BRAND.b);
    doc.text("1. SÍNTESE ACUMULADA DE INCÊNDIOS", 40, currentY);
  ...
  if (category === "todos" || category === "outras") {
    ...
    doc.text(
      category === "todos"
        ? "2. SÍNTESE ACUMULADA DE ATENDIMENTOS E OCORRÊNCIAS DIVERSAS"
        : "SÍNTESE ACUMULADA DE ATENDIMENTOS E OCORRÊNCIAS DIVERSAS",
      40,
      currentY,
    );
  ...
  if (category === "todos" || category === "efetivo" || category === "recursos") {
    doc.setFont("helvetica", "bold");
    doc.setFontSize(10);
    doc.setTextColor(BRAND.r, BRAND.g, BRAND.b);
    doc.text(
      category === "todos"
        ? "3. MOBILIZAÇÃO OPERACIONAL: EFETIVO E RECURSOS EMPREGADOS"
        : category === "efetivo"
        ? "SÍNTESE ACUMULADA DE EFETIVO OPERACIONAL EMPREGADO"
        : "SÍNTESE ACUMULADA DE RECURSOS OPERACIONAIS EMPREGADOS",
      40,
      currentY,
    );
  ...
  if (category === "todos") {
    ...
    doc.setTextColor(BRAND.r, BRAND.g, BRAND.b);
    doc.text(
      `GRANDE TOTAL OPERACIONAL ACUMULADO (INCÊNDIOS + OCORRÊNCIAS): ${NF.format(grandTotalAll)} INTERVENÇÕES`,
      40,
      currentY,
    );
  }
...
export function exportAnnualIncendiosPdf(
  years: AnnualYearSummary[],
  category: ExportCategory = "todos",
) {
  const labels = years.map((y) => y.year).join("-");
  const suffix = category === "todos" ? "completo" : category;
  buildAnnualIncendiosDoc(years, category).save(`resumo-${suffix}-${labels}-detalhado.pdf`);
}

export function exportConsolidatedIncendiosPdf(
  years: AnnualYearSummary[],
  category: ExportCategory = "todos",
) {
  const labels = years.map((y) => y.year).join("-");
  const suffix = category === "todos" ? "completo" : category;
  buildConsolidatedIncendiosDoc(years, category).save(`resumo-${suffix}-${labels}-consolidado.pdf`);
}
